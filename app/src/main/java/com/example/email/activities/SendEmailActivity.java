...
public class SendEmailActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        expandFrom = findViewById(R.id.expandFrom);
...
//                chip.setChipBackgroundColor(ColorStateList.valueOf(color));
            ...
            case R.id.sendEmailConfirm:
                ...
                if (accountIsSelected()) {
                    if (!isToAddressEmpty()) {
                        ...
                        Toast.makeText(this, "Please add at least one email address to which you want to send new email!", Toast.LENGTH_SHORT).show();

                } else
                    Toast.makeText(this, "Please select email from which you want to send new email!", Toast.LENGTH_SHORT).show();
                return true;
    ...
    private Message createMessageFromData() {
        ...
        if (chipGroupCC.getChildCount() > 0) {
            ArrayList<String> ccAddress = extractArrayListFromChipGroup(chipGroupCC);
            newMessage.setCc(ccAddress);
        }

        if (chipGroupBCC.getChildCount() > 0) {
            ArrayList<String> bccAddress = extractArrayListFromChipGroup(chipGroupBCC);
            newMessage.setBcc(bccAddress);
        }

        if (chipGroupTags.getChildCount() > 0) {
            ArrayList<Tag> tags = extractTagsFromChipGroup();
            newMessage.setTags(tags);
        }

        String textSubject = subject.getText().toString();
        newMessage.setSubject(textSubject);
            ...
            public void onResponse(Call<Boolean> call, Response<Boolean> response) {
                ...
                if (response.code() == 200) {
                    ...
                    if (resultOfSendingNewMessage) {
                        ...
                        Toast.makeText(getApplicationContext(), "Error on server, please check all receiving addresses!", Toast.LENGTH_LONG).show();
        ...
        try {
            ...
            call.enqueue(new Callback<ResponseBody>() {
                ...
                public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                    Log.d("A", "response caught");
                }

                @Override
                public void onFailure(Call<ResponseBody> call, Throwable t) {
                    Log.d("B", "response failure");
